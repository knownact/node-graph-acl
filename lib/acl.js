...
ACL.prototype.roleUsers = function(roleId, callback) {
  var _this = this;
  var users = [];

  _this.connector.getNodesWithLabelsAndProperties(['Role'], {rid: roleId}, function(err, nodes) {
    if(!nodes[0]) {
      return callback(null, []);
    }
    ...
    _this.connector.getNodeRelationships(nodes[0]._id, {types: ['BELONGS_TO']}, function(err, relationships) {
      async.map(
        relationships, // Array
        function(relationship, cb){
          _this.connector.getNode(relationship._start, function(err, userNode) {
            if (typeof userNode.uid !== 'undefined') {
              users.push(userNode);
            }
            cb(null, true);
          });
 ...
 * @param {Function} callback - Callback called when finished.
 */
ACL.prototype.whatResources = function(role, permissions, callback){
  var _this = this;
  var roleResources = {};
